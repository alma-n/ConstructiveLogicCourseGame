...
□-merkkiä käytetään ilmaisemaan, että todistus on valmis.

## Oliot ja oletukset

Lean esittää näytön alareunassa tärkeää tietoa tehtävän tilasta.
Tehtävän tila riippuu siitä millä rivillä editointikursori on.
...
**Assumptions**-otsikon alla on tehtävänannossa esitetyt oletukset ja niiden tyypit. Oletus `h` on tyyppiä `Socrates_is_human`, eli se on todistus väitteestä Socrates on ihminen.
